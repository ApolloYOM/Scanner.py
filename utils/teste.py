...
class ManualImageWarper():
    def __init__(self, current_img, file_path, main_window):
        self.main_window = main_window
        self.file_path = file_path
        self.img = cv2.imread(current_img)

        self.h, self.w, _ = self.img.shape
        a_ratio = self.w / self.h

        self.h_resized = self.main_window.img_display.height()
        ...
        main_window.img_display.mousePressEvent = self.on_mouse_clicked
        main_window.img_display.mouseReleaseEvent = self.mouse_release_event
        main_window.img_display.mouseMoveEvent = self.mouse_move_event
        # main_window.resizeEvent = self.resize_event
        main_window.keyPressEvent = self.enter_pressed
        main_window.resizeEvent = main_window.resizeEvent


        # cv2.imshow("Img", self.img_resized,)
    ...
    def update_image(self):
        ...
        self.main_window.pixmap = QPixmap.fromImage(q_img)
        self.main_window.img_display.setPixmap(self.main_window.pixmap)
        


    # def resize_event(self):
        # Resize the image to fit the width of the window when resized

        # self.img_display.setPixmap(self.pixmap.scaledToWidth(self.main_window.left_frame_a.width()))
    ...
    def on_mouse_clicked(self, event):
        if event.button() == qtc.Qt.MouseButton.LeftButton:
            mouse_pos = event.pos()
            print("Mouse Press Event - Position:", mouse_pos)
            for i, circle in enumerate(self.circles):
                center_x, center_y = circle
                distance_squared = (mouse_pos.x() - center_x) ** 2 + (mouse_pos.y() - center_y) ** 2
                print("Circle:", i, "- Center:", center_x, center_y, "- Distance Squared:", distance_squared)
                if np.sqrt(distance_squared) <= 32:
                    self.active_circle = i
                    self.dragging_offset = mouse_pos - qtc.QPoint(center_x, center_y)
                    print("Circle", i, "selected for dragging.")
                    break
            self.update_image()
    ...
    def mouse_move_event(self, event):
        if self.active_circle is not None:
            ...
            print("Mouse Move Event - New Center:", new_center.x(), new_center.y())
            print("Mouse Move Event - Active Circle:", self.active_circle)
            self.circles[self.active_circle] = [new_center.x(), new_center.y()]
            self.update_image()

    def mouse_release_event(self, ev):
        self.active_circle = None
        print("Mouse Release Event - Active Circle:", self.active_circle)
        self.update_image()
    ...
    def enter_pressed(self, event):
        if event.key() == qtc.Qt.Key.Key_Return:
            if all(circle is not None for circle in self.circles):
                img_warped = self.warp_perspective()
                img_warped_resized = cv2.resize(img_warped, (self.w_resized, self.h_resized))
                q_img = self.convert_cv_to_qt(img_warped_resized)
                self.main_window.pixmap = QPixmap.fromImage(q_img)
                self.main_window.img_display.setPixmap(self.main_window.pixmap)
