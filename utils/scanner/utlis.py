    ...
    if len(lables) != 0:
        ...
        for d in range(0, rows):
            for c in range (0,cols):
                cv2.rectangle(ver,(c*eachImgWidth,eachImgHeight*d),(c*eachImgWidth+len(lables[d][c])*eachImgWidth,30+eachImgHeight*d),(255,255,255),cv2.FILLED)
                cv2.putText(ver,lables[d][c],(eachImgWidth*c+10,eachImgHeight*d+20),cv2.FONT_HERSHEY_TRIPLEX,0.7,(255,0,255),2)
    return ver
...
def drawRectangle(img, biggest, thickness):
    # Draw each line of the rectangle with increased thickness
